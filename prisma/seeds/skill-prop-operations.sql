...
在可用技能列表中查找：名称含 `-prop-gen` 的专属道具生成技能。如有，调用 `use_skill` 加载，获取该小说类型下的领域知识指导。

领域知识在各操作中的作用：
- **创建**：指导道具属性的生成，确保符合该小说类型的惯例和风格
- **修改**：指导属性值的调整，确保修改后的内容与小说类型风格一致
- **删除**：辅助影响评估，帮助判断删除后对道具体系和剧情线索的影响
...
- 如有需要，调用 `get_plot_overview` 理解剧情需求，查看相关角色和场景信息

**Step 3**：分析 Schema 字段并生成道具数据
...
**道具 ↔ 角色**（持有/使用关系）：
- 用 `categoryA: "prop"`, `categoryB: "character"`（或反向）

**道具 ↔ 场景**（存放/出现关系）：
- 用 `categoryA: "prop"`, `categoryB: "scene"`（或反向）

**道具 ↔ 组织**（归属关系）：
- 用 `categoryA: "prop"`, `categoryB: "organization"`（或反向）

**道具 ↔ 道具**（关联关系）：
- 用 `categoryA: "prop"`, `categoryB: "prop"`
...
- 删除操作不可逆
- 关键道具（剧情核心道具、伏笔线索道具）的删除需在影响评估中重点标注风险
...
Search the available skills list for one whose name contains `-prop-gen`. If found, call `use_skill` to load it and obtain domain knowledge guidance for the specific novel type.

How domain knowledge applies across operations:
- **Create**: Guides prop attribute generation to ensure compliance with the novel type''s conventions and style
- **Modify**: Guides attribute value adjustments to ensure modified content remains consistent with the novel type''s style
- **Delete**: Assists impact assessment, helping evaluate the effect of deletion on the prop system and plot threads
...
- If needed, call `get_plot_overview` to understand plot requirements, and review related character and scene information

**Step 3**: Analyze Schema fields and generate prop data
...
- Use `categoryA: "prop"`, `categoryB: "character"` (or reversed)

**Prop <-> Scene** (storage/appearance relationships):
- Use `categoryA: "prop"`, `categoryB: "scene"` (or reversed)

**Prop <-> Organization** (affiliation relationships):
- Use `categoryA: "prop"`, `categoryB: "organization"` (or reversed)

**Prop <-> Prop** (association relationships):
- Use `categoryA: "prop"`, `categoryB: "prop"`
...
- Deletion of key props (core plot props, foreshadowing clue props) requires highlighting risks in the impact assessment
