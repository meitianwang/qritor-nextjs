...
**Step 4**：在 `create_chapter` 中通过 `characterIds`/`sceneIds`/`organizationIds`/`propIds` 直接设置参与实体
...
- 按顺序逐章调用 `create_chapter`，创建时通过 `characterIds`/`sceneIds`/`organizationIds`/`propIds` 直接设置参与实体
- 每章的大纲要与前一章自然衔接
...
- 如果大纲涉及角色/场景变动，同时通过 `characterIds`/`sceneIds`/`organizationIds`/`propIds` 更新参与实体，避免额外调用
...
> 通常在 `create_chapter` 或 `update_chapter_outline` 中直接设置参与实体即可。仅在需要单独调整实体（不改大纲）时使用 `update_chapter_entities`。

#### 4.1 何时使用

- 发现已有章节缺少参与实体
- 角色或场景发生变动，需要批量调整多个章节的实体关联
...
**Step 4**: Set participating entities directly in `create_chapter` via `characterIds`/`sceneIds`/`organizationIds`/`propIds`
...
- Call `create_chapter` sequentially for each chapter, setting participating entities directly via `characterIds`/`sceneIds`/`organizationIds`/`propIds`
...
- If the outline involves character/scene changes, update participating entities simultaneously via `characterIds`/`sceneIds`/`organizationIds`/`propIds` to avoid extra calls
...
### IV. Standalone Entity Management

> Normally, set participating entities directly in `create_chapter` or `update_chapter_outline`. Use `update_chapter_entities` only when adjusting entities independently (without changing the outline).

#### 4.1 When to Use

- Existing chapters are missing participating entities
- Characters or scenes have changed, requiring batch entity updates across multiple chapters
