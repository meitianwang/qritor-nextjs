...
1. **Research before action**: Gather sufficient context information before any character operation
...
- `get_setting_detail`: Review world-building settings to ensure the character''s background is reasonable
...
- Call `get_plot_overview` to understand the plot direction
- If needed, review relevant settings and chapter content

**Step 3**: Build character data
...
- Nested `object` or `array` type fields must recursively follow their sub-Schema
...
- For each relationship pair, call `create_relation` with:
  - `fromName`: Source character name of the relationship
  - `toName`: Target character name of the relationship
  - `relationType`: Relationship type
  - `description`: Specific description of the relationship
  - `strength`: Relationship strength (0-100, optional)

#### 2.3 Notes

- If the user does not provide enough information to fill required fields, **proactively ask** the user for more details
...
| Identity or occupation changes | Affects social relationships | Update related relation relationType |
...
- Deletion of key characters (protagonists, core supporting characters) must be fully confirmed with the user
...
| `create_relation` | Create a relationship | `fromName`, `toName`, `relationType`, `description?` |
