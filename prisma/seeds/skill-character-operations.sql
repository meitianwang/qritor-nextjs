...
- 对每对关系调用 `create_relation`，设置：
  ...
  - `nameB`：实体 B 名称
  - `relation`：关系描述（自由文本）
  - `color`：关系颜色（必填），pink/green/red/purple/blue/yellow/rose/teal
  - `description`：关系的具体描述
...
| `create_relation` | 创建关系 | `nameA`, `nameB`, `relation`, `color` |
...
- For each relationship pair, call `create_relation` with:
  ...
  - `color`: Relationship color (required), pink/green/red/purple/blue/yellow/rose/teal
...
| `create_relation` | Create a relationship | `nameA`, `nameB`, `relation`, `color` |
