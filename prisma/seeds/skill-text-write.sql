...
写入完成（`end_chapter_write` 调用后，内容自动保存），先检查正文质量并修正问题，确保正文定稿后再进行实体和剧情管理。
...
```
read_chapter_section（逐段审读，检查内容质量） → get_chapter_with_context（确认大纲覆盖、前章衔接） → get_character_detail（抽查关键角色一致性）
```

**重要**：必须在正文完全定稿后，再进入第四步的实体与剧情管理。否则正文修改会导致已更新的实体数据与正文不一致。

---

### 四、写作后：实体与剧情管理

正文检查并修正完毕后，依据最终正文内容更新项目的实体、关系、动态信息和剧情数据。
...
| `create_relation` | 创建实体关系 | `nameA`, `nameB`, `relation` |
...
### III. Post-Writing Review

After writing is complete (`end_chapter_write` called, content is auto-saved), first check body text quality and fix any issues. The text must be finalized before proceeding to entity and plot management.

#### 3.1 Content Quality Checks
...
#### 3.2 Narrative Consistency Checks
...
```
read_chapter_section (review section by section, check content quality) → get_chapter_with_context (confirm outline coverage, previous chapter continuity) → get_character_detail (spot-check key character consistency)
```

**Important**: Entity and plot management (Step IV) must only begin after the body text is fully finalized. Otherwise, text modifications will cause already-updated entity data to become inconsistent with the text.

---

### IV. Post-Writing: Entity & Plot Management

After the body text has been reviewed and finalized, update project entities, relationships, dynamic info, and plot data based on the final body text content.

#### 4.1 Manage Participating Entities
...
#### 4.2 Update Entity Relationships
...
#### 4.4 Storyline & Plot Event Management
...
| `create_relation` | Create entity relationship | `nameA`, `nameB`, `relation` |
