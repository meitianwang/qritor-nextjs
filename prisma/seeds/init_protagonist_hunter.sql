...
SET @method_id = (SELECT id FROM novel_creation_method WHERE name = 'The Protagonist Hunter' LIMIT 1);
...
-- 3. Verify
-- ============================================================
SELECT id, name, novel_type, language FROM novel_creation_method WHERE name = 'The Protagonist Hunter';
SELECT COUNT(*) AS module_count FROM module_type WHERE novel_creation_method_id = @method_id;
SELECT name, entity_category, singleton, temperature FROM module_type WHERE novel_creation_method_id = @method_id ORDER BY singleton DESC, entity_category;
