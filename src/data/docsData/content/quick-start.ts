...
2. **斜杠指令**：在输入框中输入并发送 \`/setting-generation\`。
...
写完章节后，使用 AI 助手工具栏中的提取按钮，让 AI 持续维护作品的结构化数据：
...
在与 AI 反复敲定完世界观和基础设定之后，您应该立即让 AI 帮您进行实体的初始化工作。不要手工去一个个建立角色，直接**利用界面快捷卡片、快捷指令或自然语言对话**，让 AI 基于刚刚确认的宏伟蓝图，自动为您推演和批量生成这本小说的**[核心人物](/docs/zh/user-guide/characters)**与**[主要场景](/docs/zh/user-guide/scenes)**。

**沟通案例：多次对话打磨实体与关系**：

这些自动生成的实体不仅仅是备忘录，更是 RAG (检索增强生成) 引擎在后续写作时提取上下文的唯一依据。因此，如果觉得自动填写的设定太单薄，您可以继续通过和创作助手多次对话去打磨调整人物角色、数量、关系等细节。配合各种快捷指令，Qritor 甚至会在初始化后自动构建出可视化的人物干系图。

**步骤一：启动角色库初始化**
你可以通过以下三种方式之一发起角色或各类实体的初始化请求：
1. **点击快捷建议**：当小说尚未创建任何相关的实体时，系统会在侧边栏自动为您推荐附带魔法棒图标的初始化**操作卡片**（如“一键初始化角色库”），直接点击即可。
2. **使用斜杠指令**：在聊天框输入并发送 \`/character-initialization\`（生成角色）或 \`/scene-initialization\`（生成场景）等指令。
3. **使用自然语言**：直接告诉 AI“结合前文的设定，帮我推演并生成这本小说的核心人物”。

无论哪种方式，AI 都会立刻响应，开始提取世界观大纲，并自动为您加载对应的实体生成技能。
![多样化启动角色初始化](/images/docs/examples/character-zh.png)
![初始化角色库思考过程](/images/docs/examples/character-init-thinking.png)

**步骤二：审阅核心人物阵容**
AI 会自动列出与当前世界观匹配的初始人物，并赋予其非常细致的“身份定位”与“性格特点”。
![列举初始核心人物](/images/docs/examples/character-init-cast.png)
生成完毕后，所有这些人物将自动进入左侧的模块树中，成为独立的数据节点。
![模块树中的角色库](/images/docs/examples/character-module-tree.png)

**步骤三：查看自动构建的关系图谱**
伴随角色和背景资料的丰富，AI 能够自动为您绘制不错综复杂的初始势力和人物干系网，为后续冲突埋下伏笔。
![可视化关系图谱](/images/docs/examples/character-relation-graph.png)

**步骤四：随时微调具体角色（静态与动态属性）**
如果您对某个角色（如核心反派）的属性有着更具体的设定想法，可以直接在聊天框告诉 AI 去修改，也可以点开具体卡片查看并手工编辑其详细的“基本信息”。
![编辑角色基本信息](/images/docs/examples/character-edit-info.png)
...
不要靠人脑去记忆一本书几百万字中的所有线索。小说的角色库和关系网也是随着剧情不断生长变化的。Qritor 提供了极大便利的对话交互与一键工具，用来**实时提取当前章节的结构化信息**。

- **建议您每写完一两个章节，就通过创作助手或点击 [AI 助手工具栏](/docs/zh/user-guide/ai-assistant)中的信息提取按钮**，实时执行以下维护：
  1. **提取未创建的新实体**：如果这章出现了一个路人或新反派，立刻通过 AI 为其创建全新的角色实体档案。
  2. **提取动态信息**：在角色的“动态信息”栏中，让 AI 记录人物在刚才剧情中的阵营转变、情绪变化、或者获得的特殊道具（见下图）。
  3. **实时更新关系图谱**：随着剧情进展，势力的敌友关系可能发生转变，实时让创作助手更新实体之间的人际关系。

![通过章节提取的动态信息示范](/images/docs/examples/character-dynamic-info.png)

- **这种良好的“归档习惯”正是利用系统记忆代替人脑记忆的关键**，保证后续写作时 AI 有最为准确的当前状态作为参考，长篇小说连载中再也不会出现前后矛盾的尴尬局面。
        `,
    quickLinks: [
      {
        title: "小说创作入门",
...
2. **Slash Command**: Type and send \`/setting-generation\` in the input field.
...
After writing a chapter, use the extraction buttons in the AI assistant toolbar to let the AI continuously maintain your novel's structured data:
...
Once you have locked in your overarching worldview and settings with the AI, your immediate next step is entity initialization. Please avoid creating entities manually one by one. By leveraging **UI automatic suggestion cards, slash commands, or natural language prompts**, let the AI deduce and batch-generate **[key characters](/docs/en/user-guide/characters)** and **[major scenes](/docs/en/user-guide/scenes)** based directly on your newly established blueprint. 

**Case Study: Polishing Entities and Relationships via Dialogue**

These automatically generated entity modules are not merely notes; they are the sole reference points for the RAG engine when generating contextual prose later. If you feel the generated profiles are too simple, you can continue to polish and adjust character traits, numbers, and relationships through repeated conversations with the creative assistant. It even automatically builds a visualization of character relations afterward.

**Step 1: Start Character Initialization**
You can initiate a request to initialize your entity library in one of three ways:
1. **Shortcut Selection**: When starting fresh without any relevant entities, the system will automatically recommend an action card with a magic wand icon (e.g., "1-Click Initialize Characters") in the sidebar. Just click it.
2. **Slash Commands**: Type and send \`/character-initialization\` (for characters) or \`/scene-initialization\` (for scenes) in the chat box.
3. **Natural Language**: Simply tell the AI "Please initialize the core characters for this novel based on the overall worldview we just made."

Whichever method you choose, the AI will promptly respond by extracting your worldview outline and loading specialized entity generation skills.
![Diverse Character Initialization Methods](/images/docs/examples/character-en.png)
![Character Initialization Thought Process](/images/docs/examples/character-init-thinking.png)

**Step 2: Review Core Character Cast**
The AI lists initial characters matching your selected worldview, filling in detailed "Identities" and "Personality Traits."
![Review Core Characters](/images/docs/examples/character-init-cast.png)
Once complete, all these individuals populate as independent data nodes within the left-hand Module Tree.
![Character Library in Module Tree](/images/docs/examples/character-module-tree.png)

**Step 3: View Auto-Constructed Relation Graph**
As characters and backgrounds are enriched, the AI automatically maps out complex initial factions and interpersonal networks, planting the seeds for future narrative conflicts.
![Visual Character Network](/images/docs/examples/character-relation-graph.png)

**Step 4: Tweak Specific Roles (Static and Dynamic Profiles)**
If you have specific traits or statuses in mind for a character, you can tell the AI to modify them in the chat box, or open the detailed card to view and manually edit its "Basic Info."
![Edit Basic Character Info](/images/docs/examples/character-edit-info.png)
...
Don't rely on your brain to remember every thread in a million-word novel. A novel's character library and relationship networks should organically grow and evolve alongside the plot.

- **We highly recommend using the creative AI assistant or the [extraction buttons](/docs/en/user-guide/ai-assistant)** in the AI assistant toolbar after every chapter or two to continuously:
  1. **Extract newly appearing entities**: If a new villain or side character debuts in the chapter, let the AI instantly forge their entity profile.
  2. **Extract dynamic info**: Under the character's "Dynamic Info" tab, let the AI record emotional shifts, injuries, new magical artifacts, or state changes within specific chapters (see below).
  3. **Update relationship graphs**: As the plot advances and allegiances change, tell the AI to update the relation graph among entities in real-time.

![Dynamic Info Extracted from Chapter](/images/docs/examples/character-dynamic-info.png)

- **This habit of "archiving"** is crucial for replacing human memory load with system memory, ensuring the AI always has the most accurate current state as reference, and eliminating continuity errors in long serialized fiction.
        `,
    quickLinks: [
      {
        title: "Create Novel",
