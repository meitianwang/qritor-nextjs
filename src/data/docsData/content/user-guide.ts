...
- **全局技能**：适用于所有项目，如章节操作、角色操作、剧情设计等。
- **方法专属技能**：绑定到特定创作方法（如「古代架空穿越」），仅在使用该方法的小说中可用。
...
Qritor 的技能按职责分为三层，协同完成复杂的创作任务：

### 编排层（Layer 1）

负责编排多步骤流程，协调下层技能的执行顺序。

| 技能 | 说明 |
|------|------|
| \`novel-initialization\` | 批量初始化所有设定模块 |
| \`character-initialization\` | 批量初始化所有角色 |
| \`scene-initialization\` | 批量初始化所有场景 |
| \`prop-initialization\` | 批量初始化所有道具 |
| \`organization-initialization\` | 批量初始化所有组织 |

**执行方式**：编排层技能启动后，会依次加载通用流程层技能和领域知识层技能，按模块逐个执行生成任务。
...
| 技能 | 说明 |
|------|------|
| \`setting-generation\` | 通用设定生成流程：读取模块 Schema → 生成内容 → 提交更新 |

### 领域知识层（Layer 3）

提供特定题材、特定模块类型下的专业创作知识。这些技能绑定到创作方法，由通用流程层加载使用。

**示例**（古代架空穿越方法）：
- \`ancient-crossing-worldview-setting-gen\` — 古代架空世界观生成
- \`ancient-crossing-character-gen\` — 古代架空角色生成
- \`ancient-crossing-scene-gen\` — 古代架空场景生成
- \`ancient-crossing-story-hook-setting-gen\` — 故事钩子设定生成

每种创作方法都有一套对应的 Layer 3 技能，涵盖世界观、角色、场景、组织、道具、故事钩子、写作风格等各类模块。

### 三层协作示例

以 \`/novel-initialization\` 为例：

1. **Layer 1** \`novel-initialization\` 启动，读取项目信息，获取所有设定模块列表
2. 加载 **Layer 2** \`setting-generation\`，获得通用生成流程
3. 对每个设定模块，查找匹配的 **Layer 3** 技能（如 \`ancient-crossing-worldview-setting-gen\`）
4. Layer 2 按流程执行：读取模块 Schema → 结合 Layer 3 的领域知识生成内容 → 提交更新
5. 所有模块处理完毕后，Layer 1 输出总结
...
- **Global Skills**: Available for all projects (e.g., chapter operations, character operations, plot design).
- **Method-Specific Skills**: Bound to a specific creation method (e.g., "Ancient Crossing"). Only available in novels using that method.
...
Qritor organizes skills into three layers that work together to handle complex creative tasks:

### Orchestration Layer (Layer 1)

Coordinates multi-step workflows, managing the execution order of lower-layer skills.

| Skill | Description |
|-------|-------------|
| \`novel-initialization\` | Batch-initialize all setting modules |
| \`character-initialization\` | Batch-initialize all characters |
| \`scene-initialization\` | Batch-initialize all scenes |
| \`prop-initialization\` | Batch-initialize all props |
| \`organization-initialization\` | Batch-initialize all organizations |

**How it works**: An orchestration skill starts, then sequentially loads generic process skills and domain knowledge skills to execute generation tasks module by module.

### Generic Process Layer (Layer 2)

Provides reusable, standardized workflows independent of any specific genre.

| Skill | Description |
|-------|-------------|
| \`setting-generation\` | Universal setting generation flow: read module Schema → generate content → submit update |

### Domain Knowledge Layer (Layer 3)

Provides specialized creative knowledge for specific genres and module types. These skills are bound to creation methods and loaded by the generic process layer.

**Example** (Ancient Crossing method):
- \`ancient-crossing-worldview-setting-gen\` — Ancient world worldview generation
- \`ancient-crossing-character-gen\` — Ancient world character generation
- \`ancient-crossing-scene-gen\` — Ancient world scene generation
- \`ancient-crossing-story-hook-setting-gen\` — Story hook setting generation

Each creation method has a corresponding set of Layer 3 skills covering worldview, characters, scenes, organizations, props, story hooks, writing style, and more.

### Three-Layer Collaboration Example

Using \`/novel-initialization\` as an example:

1. **Layer 1** \`novel-initialization\` starts, reads project info, and gets the list of all setting modules
2. Loads **Layer 2** \`setting-generation\` to get the universal generation workflow
3. For each setting module, finds the matching **Layer 3** skill (e.g., \`ancient-crossing-worldview-setting-gen\`)
4. Layer 2 executes the flow: read module Schema → combine with Layer 3 domain knowledge to generate content → submit update
5. After all modules are processed, Layer 1 outputs a summary
