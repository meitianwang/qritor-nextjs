...
export const zhSmartWrite: DocsContentEntry = {
  title: "AI 写作协作",
  description:
    "章节编辑器与右侧 AI 助手协作完成续写和修改，编辑器负责预览、对比和确认写入。",
  body: `
## 什么是 AI 写作协作

打开任意章节模块后，您会进入 Qritor 的章节编辑器。正文编辑、排版、大纲和参与实体管理都在这里完成；当您通过右侧 **AI 助手** 发起续写或修改时，生成结果也会回流到编辑器中，以可视化的方式供您预览和确认。

右侧 AI 助手的输入区工具栏会在选中章节时自动展示续写、润色、提取、分析等功能按钮。本文重点介绍编辑器的**核心写作能力和 AI 交互方式**，工具栏按钮的完整说明请参见 [AI 助手 - 上下文感知工具栏](/docs/zh/user-guide/ai-assistant)。

## 续写

点击 AI 助手工具栏中的 **「续写」** 按钮（快捷键 Cmd+G / Ctrl+G），AI 会根据以下上下文信息为当前章节续写正文：
...
先在编辑器中**选中一段文本**，然后点击 AI 助手工具栏中的 **「润色」** 按钮（快捷键 Cmd+P / Ctrl+P）。
...
- **AI 助手工具栏的所有按钮**都通过向 AI 助手发送指令来工作——点击按钮后，AI 助手会收到对应的提示词并开始处理
...
export const enSmartWrite: DocsContentEntry = {
  title: "AI Writing Collaboration",
  description:
    "The chapter editor works with the AI assistant on the right, while the editor handles preview, diff, and confirmation of AI writes and edits.",
  body: `
## What Is AI Writing Collaboration

When you open any chapter module, you enter Qritor's chapter editor. This is where you edit text, adjust formatting, manage outlines and participants, and review AI-generated results after triggering actions from the **AI assistant** on the right.

The AI assistant's input area toolbar on the right automatically displays buttons for continuation, polishing, extraction, analysis, and more when a chapter is selected. This article focuses on the **core writing capabilities and AI interaction patterns**. For a complete description of all toolbar buttons, see [AI Assistant - Context-Aware Toolbar](/docs/en/user-guide/ai-assistant).

## Continue Writing

Click the **"Continue"** button in the AI assistant toolbar (shortcut Cmd+G / Ctrl+G), and AI will continue writing for the current chapter based on the following context:
...
First **select a passage of text** in the editor, then click the **"Polish"** button in the AI assistant toolbar (shortcut Cmd+P / Ctrl+P).
...
The chapter editor works in deep coordination with the AI assistant on the right:

- **All buttons in the AI assistant toolbar** work by sending instructions to the AI assistant — clicking a button triggers the AI assistant to receive the corresponding prompt and begin processing
