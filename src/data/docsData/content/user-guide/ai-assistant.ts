...
输入框上方的工具栏会根据当前选中状态**自动切换显示不同的功能按钮**：

| 选中状态 | 显示的按钮 | 说明 |
|---------|-----------|------|
| **设定分组** | 设定初始化 | 一键初始化小说世界观设定 |
| **角色库/场景库/道具库/组织库分组** | 分组初始化 | 一键批量生成对应类型的实体 |
| **章节分组** | 初始化大纲 | 自动规划章节大纲 |
| **具体章节** | 续写、润色、提取实体、提取动态信息、提取关系、剧情整理、文风检测、质量评估 | 完整的章节写作 AI 工具集 |
| **具体模块**（角色/场景/道具/组织） | AI 生成 | 为选中的模块生成 AI 内容 |
| **剧情管理页面** | 初始化故事线 | 一键初始化剧情故事线 |

此外，**+（添加引用）** 按钮始终可见，用于手动添加模块引用或文字引用。

#### 章节写作按钮详解

选中具体章节时，工具栏会展示以下 AI 写作按钮（原先位于编辑器工具栏，现已迁移至此处）：

- **续写**：AI 根据上下文续写章节正文
- **润色**：需先在编辑器中选中文本，AI 优化措辞和文采
- **提取实体**：从章节正文中自动识别新出现的角色、场景等
- **提取动态信息**：识别实体在本章中的状态变化
- **提取关系**：识别实体间的关系变化
- **剧情整理**：提取关键情节点，创建剧情事件
- **文风检测**：分析文风一致性并给出建议
- **质量评估**：多维度评估章节写作质量

所有按钮在 AI 正在处理时自动禁用，避免重复操作。
...
The toolbar above the input field **automatically switches to display different buttons based on the current selection**:

| Selection State | Displayed Buttons | Description |
|----------------|-------------------|-------------|
| **Settings group** | Initialize Settings | One-click novel worldbuilding initialization |
| **Characters/Scenes/Props/Organizations group** | Initialize Group | Batch-generate entities of the corresponding type |
| **Chapters group** | Initialize Outlines | Auto-plan chapter outlines |
| **Specific chapter** | Continue, Polish, Extract Entities, Extract Dynamic Info, Extract Relations, Plot Organization, Style Check, Quality Assessment | Full chapter writing AI toolkit |
| **Specific module** (character/scene/prop/organization) | AI Generate | Generate AI content for the selected module |
| **Plot management page** | Initialize Storylines | One-click storyline initialization |

Additionally, the **+ (Add Reference)** button is always visible for manually adding module references or text quotes.

#### Chapter Writing Buttons in Detail

When a specific chapter is selected, the toolbar displays these AI writing buttons (previously located in the editor toolbar, now moved here):

- **Continue**: AI continues writing chapter prose based on context
- **Polish**: Requires text selection in the editor first; AI refines wording and style
- **Extract Entities**: Automatically identifies new characters, scenes, etc. from chapter text
- **Extract Dynamic Info**: Identifies entity state changes in the current chapter
- **Extract Relations**: Identifies relationship changes between entities
- **Plot Organization**: Extracts key plot points and creates plot events
- **Style Check**: Analyzes writing style consistency and provides suggestions
- **Quality Assessment**: Multi-dimensional chapter quality evaluation

All buttons are automatically disabled while AI is processing to prevent duplicate operations.
