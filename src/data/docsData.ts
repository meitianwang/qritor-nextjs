...
1. 在应用左下角点击 **登录** 按钮，浏览器会自动打开登录页面。
2. 在打开的网页中注册或登录账号。
3. 返回 Qritor，应用会自动跳转到小说工作区。所有创作数据存储在本地。
...
1. 在模块树中选择角色、场景、组织等模块，在编辑器中填写结构化信息。
2. 切换到 **关系图谱** Tab，查看所有实体之间的关系网络，也可以手动添加关系。
3. 你也可以跳过这一步直接开始写作，后续 AI 可以从正文中自动提取实体信息和关系。
...
1. Click the **Sign In** button at the bottom-left of the app. A browser window will open to the login page.
2. Register or sign in on the web page.
3. Return to Qritor. The app will automatically redirect to the novels workspace. All creative data is stored locally.
