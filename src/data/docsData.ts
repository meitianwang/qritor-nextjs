...
启动 Qritor 桌面客户端后，您需要先**登录账号**才能使用所有功能。
- 点击登录按钮，浏览器会自动打开登录页面。
- 完成登录后，应用会自动跳转到小说工作区。
- 所有创作数据存储在本地数据库中。
...
在侧边栏点击 **「创作方法」** 进入创作方法页面：
- 创作方法按小说题材分类（穿越、末世、灵异等），每种方法为特定题材预定义了模块类型和结构。
- 您可以搜索和浏览所有可用的创作方法，了解每种方法适合什么样的故事。
- 点击"同步"按钮可从云端获取最新的创作方法。

## 第三步：创建作品

在侧边栏点击 **「小说」** 回到作品列表，点击右上角 **「新建作品」** 按钮：
1. **作品名称**：输入作品标题。
2. **创作方法**：从下拉列表中选择一种创作方法。不同的创作方法决定了作品拥有哪些模块类型（如角色卡、场景、道具等）。
3. **简介**（可选）：输入作品简介，帮助 AI 理解您的创作意图。

创建完成后，系统会根据所选创作方法自动生成初始模块。

## 第四步：熟悉编辑器界面

点击作品卡片进入编辑器，您将看到专业的三栏布局：

### 左侧：模块树
按分类管理所有模块 — 章节、角色、场景、组织、道具和设定。

### 中间：核心工作区
包含三个 Tab 页签：
- **编辑器** — 章节的沉浸式文本编辑，或模块的表单填写。
- **关系图谱** — 可视化的实体关系网络。
- **剧情管理** — 故事线、剧情事件和伏笔追踪。

### 右侧：AI 助手
默认开启，支持 Agent 模式（可执行操作）和问答模式（仅对话）。

## 第五步：开始创作

1. 在 **模块树** 中选择一个章节或模块。
2. 在中间工作区的 **编辑器** Tab 中开始写作或填写模块信息。
3. 使用右侧 AI 助手进行续写、润色，或让 AI 帮您管理实体和剧情。
            `,
            quickLinks: []
        },
        creation: {
...
After launching the Qritor desktop client, you need to **sign in** to access all features.
- Click the login button, and a browser window will open to the login page.
- After signing in, the app will automatically redirect to the novels workspace.
- All creative data is stored in a local database on your device.

## Step 2: Browse Creation Methods

Click **"Creation Methods"** in the sidebar to explore available methods:
...
- You can search and browse all available methods to find the best fit for your story.
- Click the "Sync" button to fetch the latest creation methods from the cloud.

## Step 3: Create a Novel

Click **"Novels"** in the sidebar to return to the works list, then click the **"New Novel"** button:
1. **Novel Name**: Enter the title of your work.
2. **Creation Method**: Select a method from the dropdown. Different methods determine which module types your work will have (e.g., character sheets, scenes, props, etc.).
3. **Description** (optional): Enter a brief description to help AI understand your creative intent.

After creation, the system will automatically generate initial modules based on the selected creation method.

## Step 4: The Editor Interface

Click a novel card to enter the editor. You will see a professional three-column layout:

### Left: Module Tree
Manage all modules by category — chapters, characters, scenes, organizations, props, and settings.

### Center: Workspace Tabs
Contains three tabs:
- **Editor** — Immersive text editing for chapters, or form editing for modules.
- **Relation Graph** — Visual entity relationship network.
- **Plot Management** — Storylines, plot events, and foreshadowing tracking.

### Right: AI Assistant
Enabled by default. Supports Agent mode (can execute operations) and Ask mode (conversation only).

## Step 5: Start Writing

1. Select a chapter or module from the **Module Tree**.
2. In the center **Editor** tab, start writing or fill in module information.
3. Use the AI Assistant on the right to continue, polish, or let AI manage entities and plot for you.
            `,
            quickLinks: []
        },
        creation: {
