...
// 渲染模块字段输入框
export const ModuleFieldInput: React.FC<ModuleFieldInputProps> = ({
  fieldName,
  fieldSchema,
  parentValue,
  onChange,
    ...
    return (
      <div className="pl-3 border-l-2 border-gray-200 dark:border-gray-700 mt-2">
        ...
        {renderModuleFields(
          ...
          (newValue) => onChange({ ...parentValue, [fieldName]: newValue }),
        )}
      </div>
    );
  }
  ...
  switch (fieldSchema.type) {
    case "string":
      return (
        <input
          ...
          onChange={(e) =>
            onChange({ ...parentValue, [fieldName]: e.target.value })
          }
          placeholder={
            (fieldSchema.description as string) || `请输入${fieldName}`
          }
      ...
      return (
        <label className="flex items-center cursor-pointer">
          <input
            ...
            onChange={(e) =>
              onChange({ ...parentValue, [fieldName]: e.target.checked })
            }
    ...
    case "array": {
      ...
      const addArrayItem = () => {
        ...
        if (
          pseudoArrayField.type === "module" ||
          pseudoArrayField.type === "object"
        )
          defaultValue = {};
        ...
        onChange({
          ...parentValue,
          [fieldName]: [...arrayValue, defaultValue],
        });
      };
      ...
      return (
        <div className="space-y-2 mt-1">
          {arrayValue.map((item: unknown, index: number) => (
            <div
              key={index}
              className="flex gap-2 items-start p-3 border-2 border-dashed border-border rounded-lg"
            >
            ...
            {Object.entries(properties).map(([subKey, subSchema]) => (
              <div key={subKey}>
                <label className="block text-xs font-medium text-text-primary mb-1">
                  {subKey}
                  {(fieldSchema.required as string[])?.includes(subKey) && (
                ...
                <ModuleFieldInput
                  ...
                  onChange={(newObj) =>
                    onChange({ ...parentValue, [fieldName]: newObj })
                  }
                  allModuleTypes={allModuleTypes}
...
// 渲染数组项的输入框（根据元素类型）
export const ArrayItemInput: React.FC<ArrayItemInputProps> = ({
  field,
  value,
  onChange,
  allModuleTypes,
        ...
        return (
          <>
            {renderModuleFieldsInArray(
              field,
              (value as Record<string, unknown>) || {},
              onChange as (value: Record<string, unknown>) => void,
            )}
          </>
        );
      }
...
// 渲染模块类型字段（用于非数组）
export const ModuleFields: React.FC<ModuleFieldsProps> = ({
  field,
  value,
  onChange,
  allModuleTypes,
  ...
  const selectedModuleType = allModuleTypes.find(
    (mt) => mt.nameZh === field.moduleTypeRef,
  );
  if (!selectedModuleType || !selectedModuleType.jsonSchemaZh) {
    return (
      <div className="text-sm text-text-secondary py-2">
        未找到模块类型或该类型没有定义字段
      </div>
  ...
  try {
    const schema = JSON.parse(selectedModuleType.jsonSchemaZh);
    ...
    // 递归渲染函数
    const renderModuleFieldsRecursive = (
      f: PseudoField,
    ...
    return (
      <div className="pl-4 border-l-2 border-blue-400/30 dark:border-blue-500/30 space-y-3">
        <div className="text-xs text-blue-400 font-medium mb-2">
          模块类型: {selectedModuleType.nameZh}
        </div>
    ...
    return (
      <div className="text-sm text-red-500 py-2">解析模块类型 Schema 失败</div>
    );
  }
};

// 渲染模块类型字段（用于数组）
export const ModuleFieldsInArray: React.FC<ModuleFieldsProps> = ({
  field,
  value,
  onChange,
  allModuleTypes,
