      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        {field.type === "module" && (
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                onUpdateField(field.id, "moduleTypeRef", e.target.value)
              }
              className="admin-form-select"
            >
              <option value="">选择模块类型</option>
              {allModuleTypes.map((mt) => (
                <option key={mt.id} value={mt.nameZh}>
                  {mt.nameZh}
                </option>
              ))}
            </select>
        ...
        <div>
          ...
          <select
            ...
            onChange={(e) =>
              onUpdateField(field.id, "uiWidget", e.target.value)
            }
            className="admin-form-select"
          >
        ...
        {field.uiWidget === "textarea" && (
          <div>
            ...
            <input
              ...
              value={field.rows || 8}
              onChange={(e) =>
                onUpdateField(field.id, "rows", parseInt(e.target.value) || 8)
              }
              placeholder="8"
              className="admin-form-input"
            />
        ...
        <div className="col-span-2 flex items-center gap-6 flex-wrap">
          ...
          <label className="flex items-center cursor-pointer">
            <input
              ...
              onChange={(e) =>
                onUpdateField(field.id, "isArray", e.target.checked)
              }
              className="w-4 h-4 text-primary border-border rounded focus:ring-2 focus:ring-primary"
            />
            <span className="ml-2 text-sm text-text-secondary whitespace-nowrap">
          ...
          <label className="flex items-center cursor-pointer">
            <input
              ...
              onChange={(e) =>
                onUpdateField(field.id, "defaultCollapsed", e.target.checked)
              }
              className="w-4 h-4 text-primary border-border rounded focus:ring-2 focus:ring-primary"
            />
            <span className="ml-2 text-sm text-text-secondary whitespace-nowrap">
