...
import {
  SMART_WRITER_PRESET_FIELDS,
  createSmartWriterField,
} from "./constants";
 ...
 */
const JsonSchemaEditor = forwardRef<JsonSchemaEditorRef, JsonSchemaEditorProps>(
  ({ onSave, onClose, showToast }, ref) => {
    const [isOpen, setIsOpen] = useState(false);
    const [moduleType, setModuleType] = useState<Record<
      string,
      unknown
    > | null>(null);
    ...
    useImperativeHandle(ref, () => ({
      open: async (data: Record<string, unknown>) => {
        setModuleType(data);
        // 加载所有模块类型
        await fetchAllModuleTypes(
          data?.novelCreationMethodId as string | undefined,
        );
        // 解析现有的 jsonSchemaZh
        if (data && data.jsonSchemaZh) {
          handleParseJsonSchema(data.jsonSchemaZh as string);
        } else {
          setFields([]);
          setSchemaText("");
          setTitleField("");
    ...
    useEffect(() => {
      if (editorComponent === "SmartWriterEditor") {
        ...
        SMART_WRITER_PRESET_FIELDS.forEach((presetField) => {
          const existingIndex = newFields.findIndex(
            (f) => f.name === presetField.name,
          );
    ...
    const handleSave = async () => {
      ...
      if (duplicates.length > 0) {
        if (showToast)
          showToast("warning", `字段名称重复: ${duplicates.join(", ")}`);
        return;
      }
      ...
      try {
        const response = await authFetch(
          `/api/module-types/${moduleType!.id}`,
          {
            ...
            body: {
              ...moduleType,
              jsonSchemaZh: finalJsonSchemaStr,
            },
          },
        );
        ...
        >
          ...
          <div className="admin-modal-header" style={{ marginBottom: "16px" }}>
            ...
            >
              ...
              编辑 {moduleType?.nameZh as string}
            </h3>
