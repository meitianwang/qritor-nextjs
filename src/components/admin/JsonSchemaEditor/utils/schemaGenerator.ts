...
export function generateFieldSchema(
  ...
  if (field.isArray) {
    ...
    if (field.type === "module") {
      ...
      if (field.moduleTypeRef) {
        ...
        if (selectedModuleType && selectedModuleType.jsonSchemaZh) {
          try {
            const moduleSchema = JSON.parse(selectedModuleType.jsonSchemaZh);
      ...
      field.properties.forEach((subField) => {
        itemProperties[subField.name] = generateFieldSchema(
          subField,
          allModuleTypes,
        );
    ...
    field.properties.forEach((subField) => {
      subProperties[subField.name] = generateFieldSchema(
        subField,
        allModuleTypes,
      );
      if (subField.required) {
