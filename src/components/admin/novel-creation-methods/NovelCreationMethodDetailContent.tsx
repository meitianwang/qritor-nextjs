...
function NovelCreationMethodDetailContent() {
  ...
  const handleUpdateVisibleCategories = async (categories: string[]) => {
    ...
    try {
      ...
      if (result.code === 200) {
        setMethod((prev) =>
          prev ? { ...prev, visibleCategories: categories } : prev,
        );
  ...
  const fetchResources = async () => {
    try {
      const moduleTypesRes = await authFetch(
        `/api/module-types/novel-creation-method/${id}`,
      );
      if (moduleTypesRes.ok)
        setModuleTypes((await moduleTypesRes.json()).data || []);
    } catch (error) {
...
function MethodOverview({
  ...
  const aiModuleCount = useMemo(
    () => moduleTypes.filter((m) => m.enableAi).length,
    [moduleTypes],
  );
  const categoryCount = useMemo(() => {
    if (!method?.visibleCategories) return 5;
