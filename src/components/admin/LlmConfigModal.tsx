    ...
    if (isOpen) {
      if (isCreate) {
        ...
        setFormData({
          ...
          inputPricePerM: config.inputPricePerM ?? 0.2,
          outputPricePerM: config.outputPricePerM ?? 0.4,
    ...
    if (isCreate) {
      ...
      try {
        ...
        const response = await adminFetch(
          `${apiBasePath}/llm-configs/${config.id}`,
          {
            method: "PUT",
            body: {
    ...
    <div className="admin-modal-overlay">
      <div className="admin-modal" style={{ maxWidth: "560px" }}>
        ...
        >
          <div className="admin-form-group" style={{ marginBottom: 0 }}>
            <label className="admin-form-label">
              模型名称 {isCreate && <span style={{ color: "#ef4444" }}>*</span>}
            </label>
            <input
              type="text"
              name="modelName"
        ...
        >
          ...
          >
            积分 = ceil(inputTokens × 输入价格 + outputTokens × 输出价格) / 1M ×
            200。积分倍率（每 1M token 消耗的积分）根据价格自动计算。
          </p>
        </div>
