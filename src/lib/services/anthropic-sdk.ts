...
export function convertMessagesToAnthropicFormat(
  messages: any[],
): ConvertResult {
...
export async function* streamAnthropicContent(
  ...
  try {
    const stream = await client.messages.create({
      ...
      ...(opts.temperature != null && !opts.thinking
    ...
    for await (const event of stream as AsyncIterable<RawMessageStreamEvent>) {
      switch (event.type) {
          ...
          if (state.type === "tool_use" && state.toolCallId && state.toolName) {
            ...
            if (state.inputJson) {
              try {
                ...
                console.warn(
                  `[Anthropic] Tool args JSON parse failed: name=${state.toolName}, json=${state.inputJson}, error=${parseErr}`,
                );
                args = {};
              }
            } else {
              console.warn(
                `[Anthropic] Tool args empty: name=${state.toolName}, id=${state.toolCallId}`,
              );
            }
            console.log(
              `[Anthropic] Tool call: name=${state.toolName}, id=${state.toolCallId}, argsKeys=${Object.keys(args).join(",")}`,
            );
            yield {
              type: "tool-call",
        ...
        case "message_delta": {
          ...
          if (event.usage) {
            totalOutputTokens = (event.usage as { output_tokens: number })
              .output_tokens;
          }
          break;
        }
    ...
    console.error(
      `[Anthropic] Stream error for model=${opts.modelName}:`,
      error,
    );
