...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
function convertAssistantMessage(content: any, output: ResponseInputItem[]): void {
    ...
    const stream = await openaiClient.responses.create({
      ...
      ...(opts.reasoning ? {
        reasoning: {
          ...
          ...(opts.reasoning.summary ? { summary: opts.reasoning.summary as 'auto' | 'concise' | 'detailed' } : {}),
        },
      } : {}),
      ...(opts.temperature != null && !opts.reasoning ? { temperature: opts.temperature } : {}),
...
function extractOpenAIErrorMessage(error: InstanceType<typeof OpenAI.APIError>): string | null {
  try {
