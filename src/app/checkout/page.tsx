...
function CheckoutPageContent() {
  ...
  const fetchSavedMethods = useCallback(async () => {
    try {
      ...
      if (data.code === 200 && data.data) {
        ...
        const defaultMethod = data.data.find(
  ...
  // Restore token first, then fetch plan details and saved payment methods
  useEffect(() => {
    const init = async () => {
  ...
  const handleCheckout = async () => {
    ...
    if (!token) {
      router.push(
        "/login?redirect=" + encodeURIComponent("/checkout?plan=" + planName),
      );
      return;
    }
    ...
    try {
      ...
      if (selectedMethodId) {
        const payResponse = await authFetch(
          `/api/orders/${orderNo}/pay-with-saved`,
          {
            method: "POST",
        ...
        if (payData.code === 200 && payData.data) {
          ...
          if (payData.data.requiresAction && payData.data.clientSecret) {
            ...
            const confirmation = await stripe.confirmCardPayment(
              payData.data.clientSecret,
            );
            if (confirmation.error) {
              setError(
                confirmation.error.message || t("checkout.errors.authRequired"),
              );
              return;
            }
            ...
            if (confirmation.paymentIntent?.status === "succeeded") {
              ...
              if (
                finalizeData.code === 200 &&
                finalizeData.data?.status === "PAID"
              ) {
                router.push(`/payment/success?orderNo=${orderNo}`);
                return;
              }
              setError(
                finalizeData.message ||
                  t("checkout.errors.initiatePaymentFailed"),
              );
              return;
            }
            ...
            setError(
              payData.message || t("checkout.errors.initiatePaymentFailed"),
            );
          }
        } else {
          setError(
        ...
        if (payData.code === 200 && payData.data) {
          const checkoutUrl =
            payData.data.checkout_url || payData.data.checkoutUrl;
          if (checkoutUrl) {
        ...
        <div className="checkout-container">
          <div className="checkout-error">
            ...
            <p className="error-message">
              {error || t("checkout.errors.planNotFound")}
            </p>
            <Link href="/pricing" className="error-action">
      ...
      <div className="checkout-container">
        ...
        <div className="checkout-header">
          ...
          <p className="checkout-subtitle">
            {t("checkout.subtitle", { plan: plan.displayName || plan.name })}
          </p>
        </div>
        ...
        <div className="checkout-content">
          ...
          <div className="checkout-plan-card">
            ...
            <h3 className="plan-features-title">
              {t("checkout.featuresIncluded")}
            </h3>
          ...
          <div className="checkout-summary">
            ...
            {(savedMethods.length === 0 || selectedMethodId === null) && (
              <div className="payment-methods-preview">
                <span className="payment-methods-label">
                  {t("checkout.supportedPaymentMethods")}
                </span>
