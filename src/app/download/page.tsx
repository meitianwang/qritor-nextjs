...
export default function DownloadPage() {
    ...
    useEffect(() => {
        const fetchLatestVersion = async () => {
            try {
                const response = await fetch('https://asset.qritor.com/api/latest?platform=mac')
                if (response.ok) {
            ...
            <main className="download-page">
                ...
                <section className="download-hero">
                    <div className="container">
                        ...
                        <p className="download-hero-subtitle">{t('download.pageSubtitle')}</p>
                    </div>
                </section>
                    ...
                    <div className="container">
                        <div className="download-panel">
                            ...
                            <div className="download-selector">
                                ...
                                <p className="download-product-desc">{t('download.desktopFullDesc')}</p>
                                ...
                                <div className="qoder-platform-list">
                                    {platforms.map(platform => (
                                        <div key={platform.id} className="qoder-platform-wrapper">
                                            ...
                                            >
                                                <span className="qoder-platform-icon">{platform.icon}</span>
                                                <span className="qoder-platform-name">{platform.name}</span>
                                                {platform.badge && (
                                                    <span className="qoder-platform-badge">{platform.badge}</span>
                                                )}
                                                <span className="qoder-platform-req">{platform.requirement}</span>
                                                {platform.available && (
                                                    <span className={`qoder-platform-arrow ${expandedPlatform === platform.id ? 'expanded' : ''}`}>
                                            ...
                                            {platform.available && expandedPlatform === platform.id && platform.architectures && (
                                                <div className="qoder-arch-dropdown">
                                                    {platform.architectures.map(arch => (
                                                        <a
                                                            ...
                                                            download
                                                        >
                                                            <span className="qoder-arch-label">{arch.label}</span>
                                ...
                                <p className="download-version">{latestVersion ? `v${latestVersion}` : ''}</p>
                            </div>
                                ...
                                <div className="preview-window">
                                    ...
                                    <div className="preview-content">
                                        ...
                                        <div className="preview-main">
                                            <div className="preview-text-line w80"></div>
                                            <div className="preview-text-line w60"></div>
                                            <div className="preview-text-line w90"></div>
                                            <div className="preview-text-line w40"></div>
                                            <div className="preview-text-line w70"></div>
                                        </div>
                                        <div className="preview-ai-panel">
                                            <div className="preview-ai-header"></div>
                                            <div className="preview-ai-content"></div>
                                        </div>
                                    </div>
                                </div>
                            </div>
