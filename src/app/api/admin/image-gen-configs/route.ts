...
export async function POST(request: NextRequest) {
  try {
    ...
    const {
      ...
      supportsStyles, supports_styles,
    ...
    const config = await prisma.image_gen_config.create({
      data: {
        ...
        default_width: (defaultWidth || default_width || 1024) as number,
        default_height: (defaultHeight || default_height || 1024) as number,
        max_width: (maxWidth || max_width || 2048) as number,
        max_height: (maxHeight || max_height || 2048) as number,
        supports_styles: (supportsStyles || supports_styles || null) as string | null,
        created_at: new Date(),
        updated_at: new Date(),
      },
    })
