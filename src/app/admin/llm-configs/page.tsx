...
export default function LlmConfigPage() {
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      const response = await adminFetch(
        `/api/admin/llm-configs/${deletingId}`,
        {
          method: "DELETE",
        },
      );
  ...
  const handleTest = async (id: number) => {
    try {
      ...
      setTestResult({
        success: data.code === 200 && data.data?.success !== false,
        message:
          data.data?.error ||
          data.message ||
          (data.data?.success !== false ? "连接测试成功" : "连接测试失败"),
        details: data.data?.reply || null,
        ...
        {loading ? (
          ...
          <div className="admin-cards">
            {configs.map((config) => (
              <div
                key={config.id}
                className={`admin-card ${!config.enabled ? "opacity-50" : ""}`}
              >
                ...
                <div className="admin-card-body">
                  ...
                  <div className="admin-card-row">
                    ...
                    <span className="admin-card-value">
                      {MODEL_TIER_LABELS[config.modelTier || ""] ||
                        config.modelTier ||
                        "未设置"}
                    </span>
                  </div>
                  <div className="admin-card-row">
                    ...
                    <span className="admin-card-value">
                      入 {config.inputPricePerM}x / 出 {config.outputPricePerM}x
                    </span>
                  </div>
                  ...
                  <div className="admin-card-row">
                    ...
                    <span className="admin-card-value">
                      <span
                        className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${config.enabled ? "bg-green-100 text-green-800" : "bg-gray-100 text-gray-600"}`}
                      >
